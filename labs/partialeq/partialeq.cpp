...
bool checkMultipleError (std::vector<std::vector<long double>>& lhs,
                         unsigned long N, unsigned long& offset){

  std::vector<bool> known(N, true);
  for(unsigned long pRow = 0; pRow < N; pRow++){
    if (lhs[pRow-offset][pRow] == 0){
      known[pRow] = false;
      offset++;
    }
  }
  ...
  for(unsigned long pRow = 0; pRow < N; pRow++){
    for(unsigned long pCol = 0; pCol < N; pCol++){
      if (lhs[pRow][pCol] != 0 && !known[pCol]){
        return true;
      }
    }
  }
...
int main(void){
  ...
  while (true){
    ...
    if (checkNoneError(lhs, rhs, N)){
      std::cout << "inconsistent\n";
    } else if (checkMultipleError(lhs, N, offset)) {
      std::cout << "multiple\n";
    } else {
